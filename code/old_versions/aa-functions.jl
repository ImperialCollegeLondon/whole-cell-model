...
#     println("net change in external substrate is $var1")
    ...
    results[20]= 0#lam*N-death_rate*N #the change in num of bacterial cells, instead of just loss to dilutionr
...
#    end
    ...
    ##################################################################
    results[1]= 1000+((-et*vt*s_out/(Kt + abs(s_out)))*N)-ds*s_out +(export_rate*NH4*50)
...
#     println("export rate is $export_rate")
    export_rate=0#0.3
    AA_a_use = new_AA*2 #each AA produced uses 2 ATP molecules
    ...
    AA_dependent_rate= (10.0+ttrate)/AA #this is the first attempt at AA control
